...
(function () {
  ...
  const _COMPACT_GENES = [15,2,3,10, 9,0.01,5,0.001,1e-6, 0.01,1e-7,0.1];
  function _compactCell(layer, x, y, charge) {
    return {x:x, y:y, layer:layer, genes:_COMPACT_GENES, weights:[0,0,0,0,0,0,0,0,0],
      ...
      gradient_history:[], avg_gradient_magnitude:0, significant_gradient_change:false};
  }
  ...
  class Cell {
    ...
    shouldDie(prune, gp, pl) {
      ...
      /* Weight-magnitude pruning: if all weights are near zero, the cell passes no information */
      const wpt = CellConfig ? CellConfig.weight_prune_threshold : 0.01;
      if (prune && wpt > 0) {
        let maxW = 0;
  ...
  function getAllSettings(state, config) {
    ...
    Gradient minibatch size (K key): ${ config.gradient_minibatch_size}
    Shuffle Epochs: ${ config.shuffle_epoch}
    Weight Prune Threshold: ${ config.weight_prune_threshold}
  ...
  function main() {
    ...
    async function onKeyDown(e) {
      ...
      if (k.toLowerCase() === 'x') {
        ...
        uiPrint(`Randomized weights. avg_weights_cell auto-set to ${config.avg_weights_cell} (measured fan-in: ${measuredFanIn.toFixed(1)})`);
        return;
      }
      if (k.toLowerCase() === 'n') {
