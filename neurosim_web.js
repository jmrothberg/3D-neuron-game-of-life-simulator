...
(function () {
  ...
  /** Shown in blue panel — keep in sync with README / mnist_to_neurosim_web_json.py */
  const QUICK_START = [
    ...
    '',
    '1) Click the big white grid once (keyboard focus). Press ? to scroll this blue panel.',
    '',
    ...
    '',
    '5) Full key list: key H cycles the long help pages.',
    '',
  ...
  function setDefaultValues() {
    const c = {
      ...
      learning_rate: 0.01, bias_range: 0.01, avg_weights_cell: 5, weight_decay: 1e-6,
      charge_delta: 0.001, gradient_threshold: 1e-7, gradient_clip_range: 1,
      weight_change_threshold: 0.005, activation_slope: 0.1,
  ...
  function SimState() {
    return {
      ...
      show_3d_view: false, show_backprop_view: false, show_training_stats: false, display_set: 0,
      bingo_count: 0, max_bingo_count: 0, total_cells: 0, total_loss: 0, total_predictions: 0,
      running_avg_loss: 0, training_cycles: 0, total_weights: 0,
      total_weights_list: new Float64Array(1000),
      points: [], epochs: 1, batch_size: 1,
      ...
      reset_training_metrics() {
        ...
        this.running_avg_loss = 0; this.training_cycles = 0; this.points = [];
      },
    };
  }
  ...
  class Cell {
    ...
    initalizeAllGenes() {
      this.genes = new Array(12).fill(0);
    ...
    colorGenes() {
      ...
      colors.push([0, s9, s9], [s10, 0, s10], [s11, s11, 0]);
      this.colors = colors;
    }
    colorProteins() {
      this.protein_colors = new Array(12).fill(0);
    ...
    updateCharge(nc, dir) {
      ...
      if (dir === 'forward') {
        ...
        this.max_charge_diff_forward = Math.max(...this.forward_charges) - Math.min(...this.forward_charges);
        ...
        this.max_charge_diff_reverse = Math.max(...this.reverse_charges) - Math.min(...this.reverse_charges);
    ...
    backward(cellsArr, lr, maxBelow) {
      ...
      const up = this.getUpperLayerCells(cellsArr, reach);
      this.updateWeightsAndBias(up, lrn, reach, wd);
    }
    shouldDie(prune, gp, pl) {
      if (gp && !this.significant_gradient_change) return true;
      if (prune) {
        if (pl === 'AND' && !(this.significant_charge_change_forward && this.significant_charge_change_reverse)) return true;
        if (pl === 'OR' && !(this.significant_charge_change_forward || this.significant_charge_change_reverse)) return true;
      }
      return false;
    }
    ...
    static fromJSON(j) {
      ...
      c.colors=new Array(12).fill(0);c.protein_colors=new Array(12).fill(0);c.colorGenes();c.colorProteins();
      return c;
    }
  ...
  function backPropagation(state, config, renderBackpropFn) {
    for (let z = config.num_layers - 2; z > 0; z--) {
      let maxBelow = null;
      ...
      for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) {
        ...
        if (c) {
          c.backward(state.cells, config.learning_rate, maxBelow);
  ...
  function predictionToActual(state, config) {
    ...
    if (pg === pa) { state.bingo_count++; if (state.bingo_count > state.max_bingo_count) state.max_bingo_count = state.bingo_count; }
  }
  function trainNetwork(state, config, renderBackpropFn) {
    for (let e = 0; e < state.epochs; e++) {
      if (state.direction_of_charge_flow === '+++++>>>>>') {
        forwardPropagation(state, config);
        if (state.back_prop) backPropagation(state, config, renderBackpropFn);
        predictionToActual(state, config);
      }
      if (state.direction_of_charge_flow === '<<<<<-----') reverseForwardPropagation(state, config);
    }
  }
  function trainOnSample(state, config, sampleIndex, renderBackpropFn) {
    applyLayerFromGrid(state.cells, 0, state.training_data_layer_0[sampleIndex]);
    applyLayerFromGrid(state.cells, config.num_layers - 1, state.training_data_num_layer_minus_1[sampleIndex]);
    ...
    state.total_weights_list[sampleIndex] = state.total_weights;
    trainNetwork(state, config, renderBackpropFn);
  }

  function training(state, config, drawFn, renderBackpropFn) {
    state.bingo_count = 0;
    const setSize = config.how_much_training_data;
    for (let i = 0; i < setSize; i++) {
      trainOnSample(state, config, i, renderBackpropFn);
  ...
  function displayStatistics(cellTypes, uiPrint) {
    ...
    const lines = ['[OT IT BT] CH WG ER BI: Overcrowding Isolation Birth'];
    for (const [ct, cnt] of sorted) lines.push(`Type: ${ct} | Count: ${cnt}`);
    lines.push(`\nAverage Mutation Frequency: ${avg.toFixed(2)}`);
    uiPrint('\nDisplaying statistics for the top 5 cell types:', 10);
    uiPrint(lines.join('\n'));
  }
  function displayPhenotypeStatistics(phen, uiPrint) {
    ...
    const lines = ['Phenotype: Avg Charge | Avg Bias | Avg Error | Avg Weights | Avg Gradient | Avg Max Fwd | Avg Max Rev'];
    for (const [ph, cnt, ac, ab, ae, aw, ag, amf, amr] of rows)
      lines.push(`${ph} | Count: ${cnt} \nCharge: ${ac.toFixed(4)} | Bias: ${ab.toFixed(4)} | Error: ${ae.toExponential(4)} | Weights: ${aw.toFixed(4)} | Gradient: ${ag.toExponential(4)} | MaxFwd: ${amf.toFixed(4)} | MaxRev: ${amr.toFixed(4)}`);
    uiPrint(lines.join('\n'));
  }
  function displayMaxChargeDiff(state, config, N, uiPrint) {
    const diffs = [];
    for (let z = 1; z < config.num_layers - 1; z++)
      for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) {
        const c = state.cells[x][y][z]; if (!c) continue;
    ...
    uiPrint(diffs.slice(0, N).map(([x, y, l, cd]) => `Cell at (${x}, ${y}, ${l}) - Charge difference: ${cd.toFixed(2)}`).join('\n'));
  }
  function displayAverages(state, config, uiPrint) {
    ...
    const out = [];
    out.push(`Predictions: ${state.total_predictions} | Average Loss: ${state.running_avg_loss.toExponential(4)}`);
    ...
    for (let z = 1; z < nl - 1; z++) {
      ...
      out.push(`  Error: ${avgE[z].toExponential(4)}\n  Charge: ${avgC[z].toFixed(4)}`);
      out.push(`  Weights: ${avgW[z].toFixed(4)}\n  Avg Weights/Cell: ${avgWpc[z].toFixed(2)}`);
    }
    uiPrint('\nNetwork Statistics:', 10);
    uiPrint(out.join('\n'), 50);
  }
  function predictionPlot(state, predCtx) {
    const W = predCtx.canvas.width, H = predCtx.canvas.height;
    predCtx.fillStyle = '#000'; predCtx.fillRect(0, 0, W, H);
    const offset = 0;
    let y = H - (state.running_avg_loss / 10) * H;
    if (!Number.isFinite(y)) y = H / 2;
    state.points.push([state.points.length + offset, y]);
    if (state.points.length > 200) { state.points.shift(); for (let i = 0; i < state.points.length; i++) state.points[i][0]--; }
    predCtx.fillStyle = '#00f';
    for (let i = 0; i < state.points.length; i++) {
      const [px, py] = state.points[i];
      predCtx.beginPath(); predCtx.arc(px % W, clip(py, 0, H), 2, 0, Math.PI * 2); predCtx.fill();
    }
  }
  function updateTrainingStats(state, config) {
    ...
    for (let z = 1; z < nl - 1; z++)
      for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) {
        const c = state.cells[x][y][z]; if (!c) continue;
        tcells++; tw += c.weights.length;
      }
    const avgWpc = tw / (tcells + eps);
    const phen = updatePhenotypeCellTypes(state.cells, config);
    const geno = updateCellTypes(state.cells, config);
    const top = Object.entries(geno).sort((a, b) => b[1] - a[1]).slice(0, 5);
    let an = 0, nz = 0;
    for (let z = 1; z < nl - 1; z++)
      for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) {
        const c = state.cells[x][y][z]; if (!c) continue;
        if (c.weights.some(w => w !== 0)) an++;
        for (let i = 0; i < c.weights.length; i++) if (c.weights[i] !== 0) nz++;
      }
    const grads = [];
    for (let z = 1; z < nl - 1; z++)
      for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) {
        const c = state.cells[x][y][z]; if (c) grads.push(Math.abs(c.gradient));
      }
    state.training_stats_buffer = {
      Accuracy: `${state.bingo_count}/${config.how_much_training_data} (Max: ${state.max_bingo_count})`,
      'Avg Loss': state.running_avg_loss.toExponential(4),
      'Total Predictions': state.total_predictions,
      'Avg Weights/Cell': avgWpc.toFixed(2),
      'Total Weights': tw.toFixed(2),
      'Total Cells': tcells,
      'Active Cells': `${phen[0]}/${phen[1]}`,
      'Top Genotypes': top,
      'Active Neurons': `${an}/${tcells}`,
      'Avg Connections/Neuron': (nz / (tcells + eps)).toFixed(2),
      'Max Gradient': (Math.max(...grads, 0)).toExponential(4),
      'Avg Gradient': (mean(grads)).toExponential(4),
    };
  }
  function displayTrainingStats(state, uiPrint) {
    const b = state.training_stats_buffer;
    let s = `Training Statistics:\nAccuracy: ${b.Accuracy}\nAvg Loss: ${b['Avg Loss']}\nTotal Predictions: ${b['Total Predictions']}\n`;
    s += `Avg Weights/Cell: ${b['Avg Weights/Cell']}\nTotal Weights: ${b['Total Weights']}\nTotal Cells: ${b['Total Cells']}\n`;
    s += `Active Cells: ${b['Active Cells']}\nTop Genotypes:\n`;
    for (const [g, c] of b['Top Genotypes']) s += `  ${g}: ${c}\n`;
    s += `Active Neurons: ${b['Active Neurons']}\nAvg Connections/Neuron: ${b['Avg Connections/Neuron']}\n`;
    s += `Max Gradient: ${b['Max Gradient']}\nAvg Gradient: ${b['Avg Gradient']}\n`;
    uiPrint(s, 10);
  }
  ...
  function init3d(canvas) {
    ...
    scene.add(root);
    const matIn = new THREE.PointsMaterial({ size: 6, vertexColors: true, sizeAttenuation: true, transparent: true, opacity: 0.95 });
    const matH = new THREE.PointsMaterial({ size: 8, vertexColors: true, sizeAttenuation: true, transparent: true, opacity: 0.95 });
    const matO = new THREE.PointsMaterial({ size: 14, vertexColors: true, sizeAttenuation: true, transparent: true, opacity: 0.95 });
    ...
    g3 = { scene, camera, renderer, root, ptsIn, ptsHid, ptsOut, lines, cellRefs: [], nIn: 0, nHid: 0, nOut: 0, backpropGroup };
  }
    ...
    function putPoint(idx, x, y, z, layer, c) {
      ...
      for (let dx = -reach; dx <= reach; dx++) {
        ...
        for (let dy = -reach; dy <= reach; dy++) {
          ...
          const inten = Math.min(Math.abs(w), 1);
  ...
  function refresh3dColors(state, config) {
    ...
    for (let g = 0; g < 3; g++) {
      ...
      for (let i = 0; i < counts[g]; i++) {
        ...
        const [r, gr, b] = cell ? layerColor(z, nl, cell.charge) : [0.1, 0.1, 0.1];
  ...
  function render3dNetwork(state, config) {
    ...
    hud.textContent = [
      `Layers: ${config.num_layers} | Cells: ${g3.nIn + g3.nHid + g3.nOut}`,
      `Cycle: ${state.training_cycles} | Correct: ${state.bingo_count}/${config.how_much_training_data}`,
      `Loss: ${state.running_avg_loss.toFixed(4)} | Max: ${state.max_bingo_count}`,
      `LR: ${config.learning_rate} | Dendrite: ${config.length_of_dendrite} | BackProp: ${state.back_prop}`,
    ].join('\n');
  }
  ...
  function render3dBackprop(state, config, cz, cx, cy) {
    ...
    const addPoints = (pts, cols, size) => {
      ...
      const m = new THREE.PointsMaterial({ size, vertexColors: true, sizeAttenuation: true, depthTest: true });
    ...
    const addLines = (seg) => {
      ...
      g.setAttribute('color', new THREE.Float32BufferAttribute(new Float32Array(seg.length / 3).fill(1), 3));
  ...
  function main() {
    ...
    const quickBar = document.getElementById('quickStartBar');
    const sidePre = document.getElementById('sidePre');
    const statusLines = document.getElementById('statusLines');
    const pred = document.getElementById('pred').getContext('2d');
    ...
    let helpText = HELP_SCREEN.controls;
    function drawHelpPanel() {
      if (quickBar) quickBar.textContent = QUICK_START;
      const log = state.side_panel_text.slice(-40).join('\n');
      if (sidePre) {
        sidePre.textContent = helpText + '\n\n--- Log ---\n' + log;
      }
    }
    ...
    async function onKeyDown(e) {
      const k = e.key;
      if (k === '?' || (k === '/' && e.shiftKey)) {
        if (quickBar) quickBar.scrollIntoView({ block: 'start' });
        uiPrint('(See blue QUICK START panel above the help text.)');
        drawHelpPanel();
        return;
      }
      ...
      if (k.toLowerCase() === 'c' && !state.show_3d_view && state.charge_change_protection) {
        const a = await showModal(`Charge delta (${config.charge_delta}):`, config.charge_delta);
        const b = await showModal(`Gradient threshold (${config.gradient_threshold}):`, config.gradient_threshold);
      ...
      if (k.toLowerCase() === 'i') {
        const v = await showModal(`Learning rate (${config.learning_rate}):`, config.learning_rate);
      ...
      if (k.toLowerCase() === 'q' && !state.show_3d_view) { uiPrint(formatTelemetry(computeTelemetry(state, config)), 10); return; }
      if (k === '3') {
      ...
      if (k.toLowerCase() === 'e') {
        ...
        config.avg_weights_cell = +(await showModal('avg_weights_cell:', config.avg_weights_cell) ?? config.avg_weights_cell);
        config.weight_decay = +(await showModal('weight_decay:', config.weight_decay) ?? config.weight_decay);
        config.bias_range = +(await showModal('bias_range:', config.bias_range) ?? config.bias_range);
        config.learning_rate = +(await showModal('learning_rate:', config.learning_rate) ?? config.learning_rate);
        config.charge_delta = +(await showModal('charge_delta:', config.charge_delta) ?? config.charge_delta);
      ...
      if (k.toLowerCase() === 'v' && !state.show_3d_view) {
        ...
        if (state.display_set === 0) helpText = getAllSettings(state, config);
        else if (state.display_set === 1) displayAverages(state, config, uiPrint);
        else {
          const ct = updateCellTypes(state.cells, config);
          const pt = updatePhenotypeCellTypes(state.cells, config);
          displayStatistics(ct, uiPrint);
          displayPhenotypeStatistics(pt, uiPrint);
          displayMaxChargeDiff(state, config, 5, uiPrint);
        }
    ...
    function tick() {
      ...
      if (state.training_mode) {
        const setSize = config.how_much_training_data;
        const renderBp = state.show_3d_view && state.show_backprop_view ? doRenderBackprop : null;
        try {
          if (state.display_updating) {
            /* One sample per rAF so the browser paints each MNIST image; full loop in one tick only updates the last digit. */
            ...
            trainOnSample(state, config, i, renderBp);
            if (!state.show_3d_view) doDraw2d();
            else state._3d_dirty = true;
            if (state.training_mode && state.display_updating) predictionPlot(state, pred);
            ...
            if (state._training_sample_i >= setSize) {
              ...
              if (state.show_training_stats && state.training_cycles % state.stats_update_frequency === 0) {
                ...
                displayTrainingStats(state, uiPrint);
              }
            }
          } else {
            state._training_sample_i = 0;
            ...
            state.total_predictions = 0;
            state.training_cycles++;
            training(state, config, null, renderBp);
      ...
      statusLines.innerHTML = `<span style="color:${col}">Run=${state.running} And=${state.andromida_mode} Prune=${state.prune} ${state.prune_logic} | ` +
        ...
        `Dir=${state.direction_of_charge_flow} BackP=${state.back_prop} | t=${elapsed.toFixed(2)}s | ` +
        `Loss=${state.running_avg_loss.toFixed(4)} Bingo=${state.bingo_count} Max=${state.max_bingo_count}`;
      if (!state.training_mode && !state.show_3d_view) pred.fillRect(0, 0, pred.canvas.width, pred.canvas.height);
    }
