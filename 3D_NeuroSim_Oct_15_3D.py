...
def load_file():
    global cells, Num_Layers, NUMBER_OF_WEIGHTS, LENGTH_OF_AXON, WEIGHT_MATRIX, not_saved_yet, bingo_count, max_bingo_count, total_loss, total_predictions, running_avg_loss, training_cycles, points  
    file_dir = "./saved_states/"
...
class Cell:
    ...
    def __setstate__(self, state):
            ...
            # Add new attributes if they don't exist in the loaded state
    ...
    def update_charge(self, new_charge, direction):
        if direction == "forward":
            ...
            self.significant_charge_change_forward = self.max_charge_diff_forward > charge_delta
            ...
            self.significant_charge_change_reverse = self.max_charge_diff_reverse > charge_delta
            ...
            if andromida_mode == True:
                ...
                if cells[x, y, layer] is not None:
                    ...
                    if charge_change_protection == True:
                        if not (cell.significant_charge_change_forward or cell.significant_charge_change_reverse):  #This should be property of the cell! 
...
def training(learning_rate, reach):
    ...
    for i in range(set_size):
        ...
        # Update significant charge change flags only for the current direction
        for layer in range(1, Num_Layers-1):
            for (x, y) in np.ndindex(cells.shape[:2]):
                if cells[x, y, layer] is not None:
                    cell = cells[x, y, layer]
                    if direction_of_charge_flow == "+++++>>>>>":
                        cell.significant_charge_change_forward = cell.max_charge_diff_forward > charge_delta
                    elif direction_of_charge_flow == "<<<<<-----":
                        cell.significant_charge_change_reverse = cell.max_charge_diff_reverse > charge_delta

def reset_directional_charge_history(direction):
    for layer in range(1, Num_Layers - 1):
        for (x, y) in np.ndindex(cells.shape[:2]):
            if cells[x, y, layer] is not None:
        ...
        for (x, y) in np.ndindex(cells.shape[:2]):
            if cells[x, y, layer] is not None:  # If the cell at this position and layer is not None
                if layer == Num_Layers - 2:  # If the current layer is the second to last layer (layer 14)
                    ...
                    cells[x, y, layer].update_charge(relu(charge), "reverse")
    ...
    for layer in range(1, stop_layer):
        ...
        for (x, y) in np.ndindex(cells.shape[:2]):
            if cells[x, y, layer] is not None:  # If the cell at this position and layer is not None
                ...
                # Replace sigmoid function with ReLU function
                cells[x, y, layer].update_charge(relu(charge), "forward") # this works to get 100 out of 100 for MNEST
                #cells[x, y, layer].charge = sigmoid(charge)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_i:
                ...
                try:
                    user_input = pygame_input(f"Enter the learning rate (.01 to .001 default: {old_learning_rate:.4f}): ", old_learning_rate)
                    learning_rate = float(user_input) if user_input else old_learning_rate
